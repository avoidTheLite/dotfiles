...
4. **Install with pnpm.** Never `turbo install` (that hits a global Turbo, no lockfile, and a missing `install` task). Run `pnpm install`, then `pnpm exec turbo --version`, `pnpm test`, `pnpm dev`. Vite is 5173 (proxy to API 3000); the API uses `PORT`. `config/ports.json` and `.env.example` are the ports contract for the homelab loader, not read at runtime yet.
5. **Later apps.** `pnpm exec turbo gen frontend_app`, `pnpm exec turbo gen node_backend`, or `pnpm exec turbo gen database` (Knex + query-adapter on an existing API). Default `--example` has no database.
...
- Do not invent extra apps, databases, or Python services on first generate unless they asked for a database (`database` on the `node_backend` app).

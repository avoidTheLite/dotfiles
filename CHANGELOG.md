...
- Added a skill routing layer under `skills/` with `AGENT.md` and `Claude.md`, plus the repo-level standards that define how dotfiles packages and skill directories are authored, versioned, and installed into downstream repositories via the custom dotfiles CLI.
- Added a validation gate under `scripts/validate/check-skill-versioning.mjs` so any content change inside `skills/<skill>/` must include a version bump in that skill's `SKILL.md` frontmatter before CI passes.
